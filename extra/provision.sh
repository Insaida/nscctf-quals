...
#   update Provision will update nscCTF running in the machine.
#
# Arguments for TYPE:
...
#
# Optional Parameters:
#   -U,         --update             Pull from master GitHub branch and sync files to nscCTF folder.
...
#   -s PATH,    --code PATH          Path to nscCTF code.
#   -d PATH,    --destination PATH   Destination path to place the nscCTF folder.
...
#
# Examples:
#   Provision nscCTF in development mode:
#     provision.sh -m dev -s /home/foobar/fbctf -d /var/nscctf
#   Provision nscctf in production mode using my own certificate:
#     provision.sh -m prod -c own -k /etc/certs/my.key -C /etc/certs/cert.crt -s /home/foobar/fbctf -d /var/nscctf
#   Update current nscctf in development mode, having code in /home/foobar/fbctf and running from /var/nscctf:
#     provision.sh -m dev -U -s /home/foobar/fbctf -d /var/nscctf
...
function usage() {
  ...
  printf "  update \tProvision will update nscctf running in the machine.\n"
  printf "\nArguments for TYPE:\n"
  ...
  printf "  -d PATH     --destination PATH \tDestination path to place the nscctf folder. Default is /var/www/fbctf\n"
  ...
  printf "\nExamples:\n"
  printf "  Provision nscctf in development mode:\n"
  printf "\t%s -m dev -s /home/foobar/fbctf -d /var/nscctf\n" "${0}"
  printf "  Provision nscctf in production mode using my own certificate:\n"
  printf "\t%s -m prod -c own -k /etc/certs/my.key -C /etc/certs/cert.crt -s /home/foobar/fbctf -d /var/nscctf\n" "${0}"
  printf "  Update current nscctf in development mode, having code in /home/foobar/fbctf and running from /var/nscctf:\n"
  printf "\t%s -m dev -U -s /home/foobar/fbctf -d /var/nscctf\n" "${0}"
}
...
if [ $AVAILABLE_RAM -lt 1024 ]; then
    log "nscctf is likely to fail to install without 1GB or more of RAM."
...
if [[ "$MULTIPLE_SERVERS" == true ]]; then
    if [[ "$DOCKER" == true ]]; then
        ...
        if [[ "$SERVER_TYPE" = "hhvm" ]]; then
            ...
            if [[ -d "/vagrant" ]]; then
                ok_log 'nscctf deployment is complete! Cleaning up... nscctf will be Ready at https://10.10.10.5'
            fi
    ...
    ok_log 'nscctf deployment is complete! Cleaning up...'
fi
